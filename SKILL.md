...
description: >
  ...
  **務必在以下情境觸發**：使用者提到「互動網頁」「簡報型網頁」「HTML 報告」
  「一頁式網頁」「投影片網頁」「網頁簡報」「做成網頁」「做成 PPT 網頁」「html ppt」
  「把這個做成網頁」「轉成互動式報告」。即使使用者沒有明確說「skill」，
...
- **建議頁數**：一般 5–10 頁（封面 + 內容 + 結尾）；素材豐富可至 12 頁
...
- **轉場動畫**：[fade / slide-left / slide-up / zoom-in]
...
可用 pattern 清單與視覺效果見 `references/1.html`（索引）及各 pattern 獨立 HTML 檔。
...
- 若 pattern 名稱為 `待新增: [描述]`，先依描述自行設計，實作完成後將新 pattern 補寫一個獨立 HTML 檔存至 `references/`，並在 `references/1.html` 索引中新增連結。
